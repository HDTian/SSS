...
weight_function<-function(coords,Z , X ){    #get the smoothed and re-weighted weight function values over coords (e.g. tpoints)   #also auto re-weighted
  ...
  smoothed_log_x <- stats::ksmooth(seq_along(log_x), log_x, kernel = "normal", bandwidth = 10,#seq_along(log_x)配合bandwidth相比coords更好控制平滑度一些
                                   x.points = seq_along(log_x))$y # 指定 x.points 使得返回长度与 log_x 一致
